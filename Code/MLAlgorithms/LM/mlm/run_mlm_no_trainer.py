...
def parse_args():
    ...
    # added by me (start)
    args.output_dir = check_folder('exp', os.path.join(args.output_dir,
                                                       'Results', args.optimizer,
...
# added by me (start)
class myAcceleratedOptimizer(AcceleratedOptimizer):

    def step(self, closure=None):
        if self.gradient_state.sync_gradients:
            if self.accelerator_state.distributed_type == DistributedType.TPU:
                raise ValueError('DistributedType.TPU not implemented in myAcceleratedOptimizer')
            elif self.scaler is not None:
                raise ValueError('scaler option not implemented in myAcceleratedOptimizer')
            else:
...
class myAccelerator(Accelerator):
    ...
    def prepare_optimizer(self, optimizer: torch.optim.Optimizer, device_placement=None):
        ...
        optimizer = myAcceleratedOptimizer(optimizer, device_placement=device_placement, scaler=self.scaler)
    ...
    #             while isinstance(opt, myAcceleratedOptimizer):
    #                 opt = opt.optimizer
...
def main():
    ...
    if args.optimizer == 'SGD':
        ...
        pass
        # optimizer = AdaHessian(optimizer_grouped_parameters, lr=args.learning_rate, average_conv_kernel=True)
    elif args.optimizer == 'AdaCubic':

        eta1 = 0.05
        eta2 = 0.75
        alpha1 = 2.5  # ** ( 3)  # very successful
        alpha2 = 0.5  # ** (3)  # unsuccessful
        model.cuda()
    ...
    with open(os.path.join(args.output_dir, 'optimizer.json'), 'w') as fp:
        json.dump(optimizer.state_dict(), fp, indent=4)
    ...
    if not type(optimizer).__name__ == "AdaCubic":
        ...
        # Prepare everything with our `accelerator`.
        # We do not need lr_scheduler in AdaCubic
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            # added by me (start)
            create_graph = type(optimizer.optimizer).__name__ == "AdaCubic"
            ...
            def closure(backward=True):
                ...
                if backward:
                    ...
                    accelerator.backward(loss, create_graph=create_graph)
                return loss
            ...
            # Hugging face code below is replaced be the code above
            # with accelerator.accumulate(model):
            #     outputs = model(**batch)
            #     loss = outputs.loss
        ...
        # added by me (start)
        perplexities.append({"epoch": epoch, "perplexity": perplexity})
        # added by me (end)

        if args.output_dir is not None:
            # added-changed by me (start)
            with open(os.path.join(args.output_dir, "all_results.json"), "w") as f:
                json.dump(perplexities, f)
            # added-changed by me (end)
